...
- **Specialized vs MLLM** — val 40장 zero-shot: 앙상블 92.5% > GPT-5.4 82.5% > GPT-5.1 75.0%
...
### Specialized vs Multimodal LLM — GPT Vision 비교

"4-class 감정 분류 task" 에 한정해 우리 앙상블이 최신 멀티모달 LLM 을 얼마나 이기는지 실측 (`scripts/tests/gpt_vs_ensemble.py`). 동일한 val 40장 (class 당 10장, `seed=42`) 에서 zero-shot 으로 비교.

**프롬프트 (공정성 — GPT 에 label 정의 안 줌)**
...
| 모델 | 전체 정확도 | 격차 (앙상블 기준) | 평균 latency |
|---|---:|---:|---:|
| **HybridFER-4E 앙상블 (본 과제)** | **37/40 · 92.5%** 🥇 | — | 0.6s (warm) |
| GPT-5.4 Vision (최신) | 33/40 · 82.5% 🥈 | **-10.0%p** | 1.8s/img |
...
|---|:-:|:-:|:-:|:-:|
| 실제 (정답) | 10 | 10 | 10 | 10 |
| GPT-5.1 선택 | 7 | 12 | **14** (+4) | 7 |
| GPT-5.4 선택 | 4 | 11 | 11 | **14** (+4) |

GPT-5.1 은 **panic 편향** (anger 를 panic 으로 오인), GPT-5.4 는 **sadness 편향** (anger 를 sadness 로 오인). 세대가 바뀌어도 한국인 얼굴의 진지한 표정 해석에서 실수 패턴이 존재. 우리 specialized 앙상블만 anger 10장 전부 정확히 잡음.

#### 공정성 한계 (명시)

- **zero-shot 조건** — GPT 에 label 정의 (예: "panic=당황") 안 줌. few-shot 으로 힌트 주면 GPT 성능 일부 상승 가능
- **"panic" 단어 모호성** — 영어권에서는 극도의 공포로 해석되는 경향, 우리 라벨은 당황·놀람 포함. GPT 에 구조적 불리
- **전처리 차이** — 앙상블은 MTCNN face crop 적용 (학습 조건 매칭), GPT 는 원본 이미지 그대로. 각자 자연 조건 사용
- **샘플 40장** — 통계적 CI 넓음. 400장 확장 시 절대 수치 미세 이동 가능성 있음

#### 결론

좁고 정의된 4-class 감정 task 에서, task-specialized 앙상블이 zero-shot 멀티모달 LLM 을 **10~17.5%p 차이로 우세**. 최신 세대 (GPT-5.4) 도 우리 모델을 따라오지 못함. 일반성 trade-off 의 구조적 결과이며, 본 과제의 도메인 특화 설계가 정당화되는 근거.

전체 결과 JSON: `results/gpt_vs_ensemble.json` (5.1), `results/gpt54_vs_ensemble.json` (5.4).
...
| Version | Date | 주요 변경 |
|---|---|---|
| 2026-04-e10 | 2026-04-19 | Specialized vs Multimodal LLM 실측 — 앙상블 92.5% > GPT-5.4 82.5% > GPT-5.1 75.0% (+10~17.5%p 우세, val 40장 zero-shot). |
